 ...
 * @since 1.0.0
 */
class OnpLicensing324_LicenseManagerPage extends FactoryPages320_AdminPage  {
    ...
    public function __construct( $plugin) { 
        ...
        if ( !$this->menuTitle ) $this->menuTitle = __('License Manager', 'onp_licensing_324');
    ...
    public function assets($scripts, $styles) {
        ...
        $this->styles->add(ONP_LICENSING_324_URL . '/assets/css/license-manager.css');
        $this->scripts->add(ONP_LICENSING_324_URL . '/assets/js/license-manager.js');   
    }
    ...
    public function indexAction() {
        ...
        if ( isset( $_POST['licensekey'] ) ) {
            ...
            if ( empty( $licenseKey ) ) {
                $error = new WP_Error('FORM:KeyEmpty', __('Please enter your license key to continue.', 'onp_licensing_324'));
            } else {
            ...
            <?php } else { ?>
                <div class="license-message <?php echo $licenseData['Category'] ?>-license-message">
                    ...
                    <div class="alert alert-normal">
                        <strong><?php _e('The updates have been checked successfully.', 'onp_licensing_324') ?></strong>
                        <p>
                        <?php if ( $updatesManager->isActualVersion() ) { ?>
                            <?php _e('You use the actual version of the plugin.', 'onp_licensing_324') ?>
                        <?php } else { ?>
                            <?php if ( $updatesManager->needChangeAssembly() ) { ?>
                            <?php printf( 
                                    __('You need to upgrade to the %1$s version. <a href="plugins.php">Click here</a> to get the update.', 'onp_licensing_324'), $this->plugin->license->build  ) ?>
                            <?php } else { ?>
                            <?php printf( 
                                    __('The %1$s version is available to download. <a href="plugins.php">Click here</a> to get the update.', 'onp_licensing_324'),
...
if ( !in_array( $sociallocker->license->type, array( 'paid' ) ) ) {
 ?>
                        <a href="<?php onp_licensing_324_purchase_url( $this->plugin ) ?>" id="purchase-premium">
                            <span class="btn btn-gold btn-inner-wrap">
                            <?php if ( !empty( $this->purchasePrice ) ) { ?>
                            <i class="fa fa-star"></i> <?php printf( __('Get Key For %1$s Only', 'onp_licensing_324'), $this->purchasePrice ) ?> <i class="fa fa-star"></i></i>
                            <?php } else { ?>
                            <i class="fa fa-star"></i> <?php _e('Get Premium Key', 'onp_licensing_324') ?> <i class="fa fa-star"></i></i>
                            <?php } ?>
                            </span>
                        </a> 
                        <?php 
 ...
 ?>   
                    ...
                    <p><?php _e('Your current license:', 'onp_licensing_324') ?></p>
                    <?php } else { ?>
                    <p><?php printf( __('Your current license for %1$s:', 'onp_licensing_324'), $this->plugin->pluginTitle ) ?></p>   
                    <?php } ?>
                    <div class="license-details-block <?php echo $licenseData['Category'] ?>-details-block">
                        
                        <?php if ( $licenseManager->hasKey() ) { ?>
                        <a href="<?php $this->actionUrl('deleteKey') ?>" class="btn btn-default btn-small license-delete-button"><i class="icon-remove-sign"></i> <?php _e('Delete Key', 'onp_licensing_324') ?></a>
                        <?php } ?>
                        ...
                        <?php if ( $licenseManager->data['Build'] == 'premium' ) { ?>
                            <?php if ( $licenseManager->data['Category'] == 'free' ) { ?>
                                ...
                                <p>
                                    <?php _e('Please, activate the plugin to unlock its features. Enter the key you received with the plugin into the form below. Don\'t know where the key is? <a href="#" id="open-faq">Click here</a>.', 'onp_licensing_324') ?>
                                </p>
                                <?php } else { ?>
                                <p>
                                    <?php _e('Please, activate the plugin to unlock its features. Enter the key you received with the plugin into the form below.', 'onp_licensing_324') ?>
                                </p>
                                <?php } ?>
                                <?php if ( $this->trial ) { ?>
                                <p class="activate-trial-hint">
                                    <?php printf( __('Also you can activate a <a href="%1$s">trial version</a> for 7 days to test the plugin on this site.', 'onp_licensing_324'), $this->getActionUrl('activateTrial') ) ?>
                                </p>
                                <?php } ?>
                            <?php } else { ?>
                        ...
                        <?php } else { ?>
                            <?php if ( $licenseManager->data['Category'] == 'free' ) { ?>
                                ...
                                <p class="activate-trial-hint">
                                    <?php printf( __('You can <a href="%1$s">activate</a> a premium version for a trial period (7 days).', 'onp_licensing_324'), $this->getActionUrl('activateTrial') ) ?>
                                    <?php printf( __('Or click <a target="_blank" href="%1$s">here</a> to learn more about the premium version.', 'onp_licensing_324'), onp_licensing_324_get_purchase_url( $this->plugin ) ) ?>
                                </p>
                            <?php } else { ?>
                                <?php echo $licenseData['Description'] ?>
                            <?php } ?>
                        ...
                        <table class="license-params" colspacing="0" colpadding="0">
                            <tr>
                                <td class="license-param license-param-domain">
                                    ...
                                    <span class="license-value-name"><?php _e('domain', 'onp_licensing_324') ?></span>
                                </td>   
                                <td class="license-param license-param-version">
                                    <span class="license-value"><?php echo $this->plugin->version ?> <small><?php _e('version', 'onp_licensing_324') ?></small></span>
                                    <?php if ( $updatesManager->isVersionChecked() ) { ?>
                                        <?php if ( $updatesManager->isActualVersion() ) { ?>
                                            <span class="license-value-name"><?php _e('up-to-date', 'onp_licensing_324') ?></span>
                                        <?php } else { ?>
                                            <?php if ( $updatesManager->needChangeAssembly() ) { ?>
                                            <span class="license-value-name">
                                                <a href="plugins.php" class="link-to-upgrade">
                                                <?php printf( __('upgrade to %s', 'onp_licensing_324'), $this->plugin->license->build ) ?>
                                                </a>
                                            </span>    
                                            <?php } else { ?>
                                            <span class="license-value-name">
                                                <a href="plugins.php" class="link-to-upgrade">
                                                <?php echo $updatesManager->lastCheck['Build'] ?>-<?php echo $updatesManager->lastCheck['Version'] ?> <?php _e('available', 'onp_licensing_324') ?>
                                                </a>
                                            </span>
                                            <?php } ?>
                                        <?php } ?>
                                    <?php } else { ?>
                                    <span class="license-value-name"><span><?php _e('up-to-date', 'onp_licensing_324') ?></span></span>
                                    <?php } ?>
                                </td>  
                                <td class="license-param license-param-days">
                                    <span class="license-value"><?php echo $licenseManager->data['Build'] ?></span>                                   
                                    <span class="license-value-name"><?php _e('assembly', 'onp_licensing_324') ?></span>
                                </td>
                                <td class="license-param license-param-days">
                                    <?php if ( $licenseManager->isExpired() ) {?>
                                        <span class="license-value"><?php _e('EXPIRED!', 'onp_licensing_324') ?></span>
                                        <span class="license-value-name"><?php _e('please update the key', 'onp_licensing_324') ?></span>
                                    <?php } else { ?>  
                                        <span class="license-value">                               
                                            <?php if ( $isInfinity ) {?><?php _e('infinity', 'onp_licensing_324') ?><?php } else { ?>
                                                <?php echo $remained; ?><small> <?php _e('day(s)', 'onp_licensing_324') ?></small>
                                            <?php } ?>
                                        </span>
                                        <span class="license-value-name"><?php _e('remained', 'onp_licensing_324') ?></span>
                                    <?php } ?>    
                                </td>
                            </tr>
                        </table>
                ...
                <div class="license-input">
                    <form action="<?php $this->actionUrl("index") ?>" method="post">
                        ...
                        <?php if ( $this->premium ) { ?>
                            <p><?php _e('Have a key to activate the premium version? Paste it here:', 'onp_licensing_324') ?><p>
                        <?php } else { ?>
                            <p><?php _e('Have a key to activate the plugin? Paste it here:', 'onp_licensing_324') ?><p>
                        <?php } ?>
                            
                        <a href="#" class="btn btn-default" id="license-submit">
                            <?php _e('Submit Key', 'onp_licensing_324') ?>
                        </a>  
                        ...
                        <?php if ( $this->premium ) { ?>
                            <p style="margin-top: 10px;">
                                <?php printf( __('<a href="%1$s">Lean more</a> about the premium version and get the license key to activate it now!', 'onp_licensing_324'), onp_licensing_324_get_purchase_url( $this->plugin ) ) ?>
                            </p>
                        <?php } else { ?>
                            <p style="margin-top: 10px;">
                                <?php printf( __('<a href="%1$s">Lean more</a> about this plugin and get the license key to activate it now!', 'onp_licensing_324'), onp_licensing_324_get_purchase_url( $this->plugin ) ) ?>
                            </p>
                        <?php } ?>
                    </form>
                </div>
                ...
                <?php if ( $licenseManager->data['Build'] !== 'free' ) { ?>
                    <?php if ( !$updatesManager->isVersionChecked() ) { ?>
                        <?php if ( isset( $updatesManager->lastCheck['Checked'] ) ) { ?>
                            <?php printf( __('The upadtes were checked at <strong>%1$s</strong>.', 'onp_licensing_324'), date( 'g:i a, j M y', $updatesManager->lastCheck['Checked'] ) ) ?>
                        <?php } else { ?>
                            <?php _e('The updates were checked <strong>never</strong>.', 'onp_licensing_324') ?>
                        <?php } ?>
                    <?php } else { ?>
                        <?php if ( $updatesManager->isActualVersion() ) { ?>
                            <?php printf( __('The updates were checked at <strong>%1$s</strong>, you use the up-to-date version. ', 'onp_licensing_324'), date( 'g:i a, j M y', $updatesManager->lastCheck['Checked'] ) ) ?>
                        <?php } else { ?>
                            <?php printf( __('The updates were checked at <strong>%1$s</strong>, <strong>%2$s</strong>. ', 'onp_licensing_324'), date( 'g:i a, j M y', $updatesManager->lastCheck['Checked'] ), $updatesManager->lastCheck['Version'] ) ?>
                        <?php } ?>
                    <?php } ?>
                    <?php printf( __('Click <a href="%1$s">here</a> to check updates manually.', 'onp_licensing_324'), $this->getActionUrl('checkUpdates') ) ?>
                <?php } ?>
                <span class="gray-link">[ <a href="<?php echo $this->getActionUrl('internalKeys') ?>"><?php _e('internal keys', 'onp_licensing_324') ?></a> ]</span>       
            </div>
                ...
                <ul>
                    <li>
                        <a class="faq-header" id="how-to-find-the-key">
                            <?php _e('I purchased the plugin on CodeCanyon, but I cannot find the license key. Where is it?', 'onp_licensing_324') ?>
                        </a>
                        <div>
                            <p>
                                <?php _e('If you purchased the plugin on CodeCanyon, visit your Downloads section and click Licence Certificate.', 'onp_licensing_324') ?>
                                <?php _e('Find Item Purchase Code in the text document and paste it into the form above.', 'onp_licensing_324') ?>
                            </p>
                            <p style="text-align: center;">
                                <img src="<?php echo ONP_LICENSING_324_URL . '/assets/img/how-to-find-key.png' ?>" />
                            </p>
                        </div>
                    </li>
                    <li>
                        <a class="faq-header">
                            <?php _e('The plugin I purchased comes with the bonus. Where I can find a key to activate the bonus?', 'onp_licensing_324') ?>
                        </a>
                        <div>
                            <p>
                                <?php _e('You can activate the bonus plugin via the key you got for the principal plugin.', 'onp_licensing_324') ?>
                            </p>
                        </div>
                    </li>
                    <li>
                        <a class="faq-header">
                            <?php _e('Is it possible to get the premium version without the License Manager?', 'onp_licensing_324') ?>
                        </a>
                        <div>
                            <p>
        ...
        <div class="factory-bootstrap-325 factory-fontawesome-320 onp-page-wrap onp-single-block" id="finish">
            ...
            <div class='onp-header'>  
                <?php if ( $ref == 'trial' || $ref == 'manual-trial-activation' ) { ?>
                    <h4><?php _e('Congratulations! you\'ve successfully activated a trial version', 'onp_licensing_324') ?></h4>
                    <p>
                        <?php printf( __('Thank you for using %s. The plugin is ready.', 'onp_licensing_324'), $this->plugin->options['title'] ) ?>
                    </p>
                <?php } elseif ( $ref == 'binding' ) { ?>    
                    <h4><?php _e('Good job!', 'onp_licensing_324') ?></h4>
                    <p><?php _e('The plugin is ready to use.', 'onp_licensing_324') ?></p>
                <?php } else { ?>
                    <h4><?php _e('Congratulations! you\'ve successfully activated your key', 'onp_licensing_324') ?></h4>
                    <?php if ( $ref !== 'manual-key-activation' ) { ?>
                        <p><?php _e('You can transfer your key to another site at any time via your customer account.', 'onp_licensing_324') ?></p>
                    <?php } ?>
                <?php } ?>
            </div>
            ...
            <div class="onp-container">
                <?php if ( $ref == 'trial' || $ref == 'manual-trial-activation' ) { ?>
                    <p>
                        <?php _e('Your trial key will expire in ', 'onp_licensing_324') ?>
                        <?php
                            $licenseData = $licenseManager->data;
                            $remained = round( ( $licenseData['Expired'] - time() ) / (60 * 60 * 24), 2 );
                        ?>
                        <?php echo $remained; ?> <?php _e('day(s).', 'onp_licensing_324') ?>
                    </p>
                <?php } elseif ( $ref == 'binding' ) { ?>  
                    <p><?php printf( __('The key has been bound to your account (<strong>%s</strong>).', 'onp_licensing_324'), $email ) ?></p>
                    <p>
                        <?php _e('You can log in to your account at any time to manage your key by using the login details sent you via email earlier:', 'onp_licensing_324') ?>
                        <a href="<?php echo $this->plugin->options['account'] ?>" target="_blank"><?php echo $this->plugin->options['account'] ?></a>
                    </p>
                <?php } else { ?>
                    <p><?php _e('Thank you for activating your license key.', 'onp_licensing_324') ?></p>
                <?php } ?>
                <?php if ( $updatesManager->needChangeAssembly() ) { ?>
                    <p style="background-color: #fafafa; padding: 10px;">
                        <?php _e('Please move to the Plugins page and download the update to complete activation.', 'onp_licensing_324') ?>
                    </p>
                ...
                <?php } else { ?>   
                    <p><i><?php _e('Please feel free to contact us if you need any help.', 'onp_licensing_324') ?></i></p>
                    <div class='onp-actions'>
    ...
    public function createAccountAction() {
        ...
        if ( isset( $_POST['submit'] ) ) {
            ...
            if ( empty($email) ) {
                $error = new WP_Error('FORM:EmailEmpty', __('Please enter your email adress to create a customer account.', 'onp_licensing_324'));
            } else {
        ...
        <div class="factory-bootstrap-325 factory-fontawesome-320 onp-page-wrap onp-single-block" id="create-account">
            ...
            <div class='onp-header'>  
                <h4><?php _e('Congratulations! you\'ve successfully activated your key', 'onp_licensing_324') ?></h4>
                <p><?php _e('You can start using the plugin right now, but we recommend you to make one more step ...', 'onp_licensing_324' ) ?></p>
            </div>
            <?php } else { ?>
            <div class='onp-header'>  
                <h4><?php _e('Your license key is not bound to your email address!', 'onp_licensing_324') ?></h4>
            </div>
            <?php } ?>   
            ...
            <div class="license-message">
              <div class="alert alert-success">
                  <h4 class="alert-heading"><?php _e('The incorrect account has been canceled successfully', 'onp_licensing_324') ?></h4>
                  <p><?php _e('Please be careful when typing your email address.', 'onp_licensing_324') ?></p>
              </div>    
            </div>
            <?php } ?>
            ...
            <div class="onp-container">
                ...
                <div class="onp-container-header">
                    <h4><?php _e('It\'s time to protect your license key', 'onp_licensing_324' ) ?></h4>
                    <div class="onp-key-info">
                        <i class="fa fa-key"></i> <?php echo $key ?> (<i><?php echo $pluginName ?></i>)
                    </div>
                </div>

                <p><?php _e('Enter your email address to bind the license key. We will create a customer account for you by using your email address. After that, you will be able to:', 'onp_licensing_324' ) ?></p>
                <ul class='onp-benefits'>
                    <li><?php _e('View where your key is used.', 'onp_licensing_324' ) ?></li>
                    <li><?php _e('Remove the key for sites where it\'s activated.', 'onp_licensing_324' ) ?></li> 
                    <li><?php _e('Transfer the key to another sites.', 'onp_licensing_324' ) ?></li>                     
                </ul>
                ...
                <div class='onp-form'>
                      ...
                      <div class="checkbox">
                          <label>
                              ...
                              <?php echo sprintf( __('I want to get news regarding the <strong>%s</strong> plugin and exclusive offers from OnePress (a few emails a month).', 'onp_licensing_324'), $this->plugin->pluginTitle ) ?>
                          </label>
                      </div>
                  </div>
                
                <div class='onp-actions'>
                    <input type="submit" class='btn btn-lg btn-large btn-primary' name="submit" value='<?php _e('Protect & Create Account', 'onp_licensing_324' ) ?>' />
                    <?php _e('(The login details will be sent via email)', 'onp_licensing_324' ) ?>
                </div>
            </div>
        </div>
        </form>
    ...
    public function accountAlreadyCreatedAction() {
        ...
        if ( isset( $_POST['submit'] ) ) {
            ...
            if ( empty($email) ) {
                $error = new WP_Error('FORM:EmailEmpty', __('Please enter your email adress.', 'onp_licensing_324'));
            } else {
        ...
        <div class="factory-bootstrap-325 factory-fontawesome-320 onp-page-wrap onp-single-block" id="account-already-created">
            ...
            <div class="onp-container">
                <p><?php _e( 'On all probability, you created a customer account in past to bind another key. If  it\'s your email, no problem, just click the confirmation button below.', 'onp_licensing_324') ?><p>
                <p><?php _e( 'If it\'s not email (you entered  your email incorrectly), return back.', 'onp_licensing_324') ?>
                <p><?php _e('Email:', 'onp_licensing_324') ?> <strong style='font-size: 16px;'><?php echo $email ?></strong>
                <p><?php printf( __('You can try to log in via this email <a href="%s" target="_blank">here</a>.', 'onp_licensing_324'), $this->plugin->options['account'] ) ?></p>
                <div class='onp-actions'>
                    <input type="submit" class='btn btn-lg btn-large btn-primary' name="submit" value='<?php _e('Yes, this is my email', 'onp_licensing_324') ?>' />
                    <a href='<?php $this->actionUrl('createAccount', array('email' => $email )) ?>' class='btn btn-lg btn-large btn-default'><?php _e('No, return back', 'onp_licensing_324') ?></a>
                </div>
            </div>
    ...
    public function accountCreatedAction() {
        ...
        <div class="factory-bootstrap-325 factory-fontawesome-320 onp-page-wrap onp-single-block" id="account-created">
            ...
            <div class='onp-header'>
                ...
                <p><?php _e('We have sent the login details and the confirmation link to your email address.', 'onp_licensing_324' ) ?></p>
            </div>
            ...
            <div class="onp-container">
                ...
                <p>
                    <?php _e('Please perform these simple final steps:', 'onp_licensing_324') ?>
                </p>
                
                <div class="onp-steps">
                    <p class="onp-step">
                        <strong class="onp-num">1</strong>
                        <span class="onp-desc"><?php printf( __('Visit your email box <strong>%s</strong>.', 'onp_licensing_324'), $email ) ?></span>
                    </p>
                    <p class="onp-step">
                        <strong class="onp-num">2</strong>
                        <span class="onp-desc"><?php printf( __('Confirm your email address by clicking the link inside the email we sent.', 'onp_licensing_324'), $email ) ?></span>
                    </p>
                </div>
                
                 <div class='onp-actions'>
                     <a href="<?php $this->actionUrl('finish', array('onp_ref' => 'binding', 'email' => rawurlencode( $email ))) ?>" class='btn btn-lg btn-large btn-primary'><?php _e('Okay, done', 'onp_licensing_324') ?></a>
                    <a class="onp-cancel" href="<?php $this->actionUrl('cancelAccount', array('email' => rawurlencode( $email ), 'cancelCode' => $cancelCode, 'confirmationId' => $confirmationId) ) ?>">
                        <i class="fa fa-trash-o"></i> <?php _e("I've mistaken. It's not my email address.", 'onp_licensing_324') ?>
                    </a>
                </div>
            </div>
        ...
        ?>
            <div class="license-message">
                ...
                <div class="alert alert-danger">
                    <h4 class="alert-heading"><?php _e('The request has been rejected by the Licensing Server', 'onp_licensing_324') ?></h4>
                    <p><?php echo $error->get_error_message() ?></p>
                </div>    
                <?php } elseif ( $errorSource == 'HTTP' ) { ?>
                <div class="alert alert-danger">
                    <h4 class="alert-heading"><?php _e('Unable to connect to the Licensing Server', 'onp_licensing_324') ?></h4>
                    <p><?php echo $error->get_error_message() ?></p>
                    <p>
                    <?php if ($action == 'submit-key' ) {      
                          printf( 
                              __('Please <a href="%s">click here</a> for trying to activate your key manualy.', 'onp_licensing_324'),
                              $this->getActionUrl('activateKeyManualy', array('key' => $_POST['licensekey'] ))
                          );
                      } elseif ($action == 'trial') {
                          printf( 
                              __('Please <a href="%s">click here</a> for trying to activate your trial manualy.', 'onp_licensing_324'),
                              $this->getActionUrl('activateTrialManualy') 
                          );
                      } elseif ($action == 'delete-key') {
                          printf( 
                              __('Please <a href="%s">click here</a> for trying to delete key manualy.', 'onp_licensing_324'),
                              $this->getActionUrl('deleteKeyManualy')
                          );
                      } else {
                          ?>
                        <i><?php _e('Please contact OnePress support if you need to perform this action.', 'onp_licensing_324') ?></i> 
                          <?php
                      } ?>
                      </p>
                </div>
                <?php } else { ?>
                <div class="alert alert-danger">
                    <h4 class="alert-heading"><?php _e('Unable to apply the specified key', 'onp_licensing_324') ?></h4>
                    <p><?php echo $error->get_error_message() ?></p>
                </div>  
                <?php } ?>
            </div>
    ...
    public function activateKeyManualyAction() {
        ...
        <div class="factory-bootstrap-325 onp-page-wrap" id="activate-key-manual">
            ...
            <div class="onp-container">
                ...
                <p style="margin-top: 0px;"><?php _e('Please perform the following steps to activate the plugin manually.', 'onp_licensing_324') ?></p>
                <ul>
                    <li>
                        1. <?php printf( __('<a href="%s" target="_blank">Click here</a> to get an activation code.', 'onp_licensing_324'), $this->requestUrl ) ?>
                    </li>
                    <li>
                        2. <?php _e('Copy the code and paste it below, then submit the form:', 'onp_licensing_324') ?>
                        <textarea name="response" class="license-reponse-code" placeholder="<?php _e('Activation Code', 'onp_licensing_324') ?>"></textarea>
                    </li>
                </ul>
                <a href="#" class="btn btn-primary" id="manual-trial-submit">
                    <i class="icon-ok-sign icon-white"></i> <?php _e('Verify Code', 'onp_licensing_324') ?>
                </a>  
            </div>
            </form>
        </div>
    ...
    public function deleteKeyManualyAction() {
        ...
        <div class="factory-bootstrap-325 onp-page-wrap" id="activate-key-manual">
            ...
            <div class="onp-container">
                ...
                <p style="margin-top: 0px;"><?php _e('Please perfome the following steps to activate the plugin manualy.', 'onp_licensing_324') ?></p>
                <ul>
                    <li>
                        1. <?php printf( __('<a href="%s" target="_blank">Click here</a> to send the deactivation request.', 'onp_licensing_324'), $this->requestUrl ) ?>
                    </li>
                    <li>
                        2. <?php _e('Copy the code from the field on the site and paste it below, then submit the form:', 'onp_licensing_324') ?>
                        <textarea name="response" class="license-reponse-code" placeholder="<?php _e('Response code from the Licensing Server', 'onp_licensing_324') ?>"></textarea>
                    </li>
                </ul>
                <a href="#" class="btn btn-primary" id="manual-trial-submit">
    ...
    public function activateTrialManualyAction() {
        ...
        <div class="factory-bootstrap-325 onp-page-wrap" id="activate-key-manual">
            ...
            <div class="onp-container">
                ...
                <ul>
                    <li>
                        1. <?php printf( __('<a href="%s" target="_blank">Click here</a> to send the activation request.', 'onp_licensing_324'), $this->requestUrl ) ?>
                    </li>
                    <li>
                        2. <?php _e('Copy the code from the field on the site and paste it below, then submit the form:', 'onp_licensing_324') ?>
                        <textarea name="response" class="license-reponse-code" placeholder="<?php _e('Response code from the Licensing Server', 'onp_licensing_324') ?>"></textarea>
