...
if ( in_array( $sociallocker->license->type, array( 'free' ) ) ) {
        ...
        $content = do_shortcode( $content );
            ...
            if (is_feed()) {
                ...
                if ( get_option('sociallocker_rss', false) ) {
                    echo $content;
                    return;
                } else {
                    return;
            ...
            if ( is_user_logged_in() && OnpSL_AssetsManager::getLockerOption($id, 'hide_for_member', false) ) {
                echo '<p>' . $content . '</p>';
                return;
            }

            if ( !empty($post) && OnpSL_AssetsManager::getLockerOption($id, 'lock_delay', false) ) {
                $lockDelayInterval = OnpSL_AssetsManager::getLockerOption($id, 'lock_delay_interval_in_seconds');
        ...
        $lockData['ajax'] = false;
            ...
            if (isset( $lockerMeta['sociallocker_ajax'] ) && $lockerMeta['sociallocker_ajax'][0] ) {
                $isAjax = true;
