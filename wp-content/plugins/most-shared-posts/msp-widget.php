...
class Most_Shared_Posts extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		while ( $posts_in_range->have_posts() ) : $posts_in_range->the_post();
			...
			$fb_shares = get_post_meta( get_the_ID(), 'facebook_shares', true );
			$plusones = get_post_meta( get_the_ID(), 'google_shares', true );
			
			echo '<li>';
			echo '<a href="' . get_permalink(). '" title="' . get_the_title() . '">';
				echo the_post_thumbnail('widgetthumb');
			echo '</a>';

			echo '<div class="most-shared-wrapper">';
			if( $h3_wrap ) {
	...
	function form( $instance ) {
		...
		<p>
			...
			<select name="<?php echo $this->get_field_name( 'recency_limit_unit' ); ?>">
				<option value="days" <?php if ($instance['recency_limit_unit'] == 'days') { echo "selected=\"selected\""; } ?>>days</option>
				<option value="months" <?php if ($instance['recency_limit_unit'] == 'months') { echo "selected=\"selected\""; } ?>>months</option>
				<option value="years" <?php if ($instance['recency_limit_unit'] == 'years') { echo "selected=\"selected\""; } ?>>years</option>
			</select>
		</p>
