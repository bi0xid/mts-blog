...
function wp_ajax_query_shortcodeclassic(ajaxParam){
	var item_showed = jQuery('#waq'+ajaxParam['waq_id']+' .wp-ajax-query-content .ajax-item').length;
        if(!item_showed){
            item_showed = jQuery('.post.excerpt').length;
        }
	if(item_showed<9999){
			...
			success: function(data){
				...
				if(data=='-11'||data.replace(/(\r\n|\n|\r)/gm,"")=='-11'){
					...
					$j('#waq'+ajaxParam['waq_id']+'.modern .wp-ajax-query-content').append(data).imagesLoaded( function(){
						wp_ajax_query_resize();
						$j('#waq'+ajaxParam['waq_id']+'.modern .wp-ajax-query-content').masonry().masonry( 'reload' );
					});
					//jQuery("a[rel^='prettyPhoto']").prettyPhoto({});
					ajax_running=0;
				}
...
$j(window).resize(function(){
	wp_ajax_query_resize();        
	$j('.modern .wp-ajax-query-content').masonry().masonry( 'reload' );
});
...
jQuery(document).ready(function(e) {
	jQuery('.ajax-layout-toggle').click(function(e){
		...
		wp_ajax_query_resize();
		jQuery(this).closest('.wp-ajax-query-shortcode').find('.wp-ajax-query-content').masonry().masonry( 'reload' );
    });
});