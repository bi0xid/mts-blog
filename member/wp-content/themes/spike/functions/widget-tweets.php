...
/* -------------------------------------------------------------------- */
	...
	function swt_Tweet_Widget() {
		...
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'swt_tweet_widget', 'description' => __('A widget for showing latest tweets in sidebar', 'mythemeshop') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'swt_tweet_widget' );

		/* Create the widget. */
		$this->WP_Widget( 'swt_tweet_widget', __('MyThemeShop: Latest Tweets','mythemeshop'), $widget_ops, $control_ops );
	}
...
/* -------------------------------------------------------------------- */
	...
	function form( $instance ) {
		...
		<!-- Username: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'username' ); ?>"><?php _e('Twitter Username e.g. mythemeshopteam', 'mythemeshop') ?></label>
			<input type="text" class="widefat" id="<?php echo $this->get_field_id( 'username' ); ?>" name="<?php echo $this->get_field_name( 'username' ); ?>" value="<?php echo $instance['username']; ?>" />
		</p>
		
		<!-- Postcount: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'postcount' ); ?>"><?php _e('Number of tweets (max 20)', 'mythemeshop') ?></label>
			<input type="text" class="widefat" id="<?php echo $this->get_field_id( 'postcount' ); ?>" name="<?php echo $this->get_field_name( 'postcount' ); ?>" value="<?php echo $instance['postcount']; ?>" />
		</p>
		
		<!-- Tweettext: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'tweettext' ); ?>"><?php _e('Follow Text e.g. Follow me on Twitter', 'mythemeshop') ?></label>
