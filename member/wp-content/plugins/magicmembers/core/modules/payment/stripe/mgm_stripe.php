...
class mgm_stripe extends mgm_payment{
	...
	function process_unsubscribe() {				
		...
		// fix
		$member = $this->_fix_cust_data($member);
		// check
	...
	function process_credit_card(){			
		...
		// action
		if( isset($data['plan']) ){
			// has a recurring plan
			if( ! empty($data['plan']) ){
				$action =  'create_customer';
			}else{
			// one time 	
				$action =  'create_customer_and_charge';
			}
		}else{
			$action = 'create_charge';// action
		}

		// echo $action;
		// check if upgrade, we can check $tran['data']['subscription_option'] == 'upgrade'
		$subscr_id = $cust_id = $input_data = null;
		// check
		if( $user_id = $tran['data']['user_id']){
			...
			if( $member = mgm_get_member($user_id) ){
				...
				if( isset($member->payment_info->module) && $member->payment_info->module == $this->module ){
					...
					if( $cust_id  ){
						// action
						if( 'create_customer_and_charge' == $action ){
		...
		// log
		mgm_log(mgm_filter_cc_details($post_data), $this->module . '_' . __FUNCTION__);
		// to object
	...
	function get_transaction_info($member, $date_format){		
		...
		// info
		$info = sprintf('<b>%s:</b><br>%s: %s<br>%s: %s', __('STRIPE INFO','mgm'), 
	 ...
	 */
	 function _fix_cust_data($member){
	 	// cust id
	...
	function _buy_membership() {	
		...
		// transaction type
		if(isset($this->response->subscription->object)){	
			$member->payment_info->txn_type = $this->response->subscription->object;	
		}else{
		...
		if(isset($this->response->id)){
			// set
			//if( preg_match('/^cus_/', $this->response->id) ){
	...
	function _remote_post($request_url, $data=array(), $options=array(), $on_error_message='CONNECT_ERROR'){
		...
		// log
		// mgm_log($request_url, $this->module . '_' .__FUNCTION__);

		// log
		// mgm_log($args, $this->module . '_' .__FUNCTION__);

	 	// request
		$request = wp_remote_post($request_url, $args);

		// log
		// mgm_log($request, $this->module . '_' .__FUNCTION__);
	...
	function _get_api_notify($action, $post_data=null, $input_data=null){
		...
		switch(  $action ) {
			...
			case 'create_charge':
				...
				// log
				mgm_log($api_url . ' ' . $http_response, $this->module . '_' . $action . __FUNCTION__);
				// return
			...
			case 'create_customer_and_charge':
				...
				// log
				mgm_log($post_data_customer, $this->module . '_' . $action . __FUNCTION__);
				// create customer
				if( $customer = $this->_get_api_notify('create_customer', $post_data_customer) ){
					...
					if( isset($customer->id) ){
						...
						// log
						mgm_log($post_data_charge, $this->module . '_' . $action . __FUNCTION__);
						// charge
						if( $charge = $this->_get_api_notify('create_charge', $post_data_charge) ){
							...
							if( $charge->paid == true ){
								...
								// log
								mgm_log($customer, $this->module . '_' . $action . __FUNCTION__);
								// return
								return $customer;							
							}
						}
			...
			case 'use_customer_and_charge':
			// create customer
				if( $customer = $this->_get_api_notify('get_customer', array('customer_id'=>$post_data['stripe_customer_id'])) ){
					// charge
					if( $charge = $this->_get_api_notify('create_charge', $post_data) ){
						// check
						if( $charge->paid == true ){
							// create a dummy node
							$customer->subscription = new stdClass;
